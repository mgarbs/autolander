...
function toVehicle(raw, feedUrl) {
  const title = raw.name || raw.title || raw.vehicleTitle || raw.description;
  const titleParts = parseTitleToParts(title);
  const vehicle = createEmptyVehicle();

  vehicle.vin = normalizeVin(raw.vin || raw.vehicleIdentificationNumber || extractVin(title));
  ...
  vehicle.model = cleanText(raw.model || raw.modelName || titleParts.model);
  vehicle.trim = cleanText(raw.trim || raw.variant || titleParts.trim);
  vehicle.price = normalizePrice(raw.price || raw.internetPrice || raw.offers?.price || raw.offers?.[0]?.price);
  vehicle.mileage = normalizeMileage(raw.mileage || raw.odometer || raw.miles);
  ...
  vehicle.photos = uniqPhotos(
    ensureArray(raw.photos || raw.images || raw.image || raw.photoUrls).map((item) =>
      typeof item === 'string' ? item : item?.url || item?.contentUrl || item?.src
    )
  );
...
function parseXmlVehicles(xml, feedUrl) {
  ...
  $('vehicle').each((_, el) => {
    const node = $(el);
    const raw = {
      vin: node.find('vin').first().text(),
      year: node.find('year').first().text(),
      make: node.find('make').first().text(),
      model: node.find('model').first().text(),
      trim: node.find('trim').first().text(),
      price: node.find('price, internet_price, asking_price').first().text(),
      mileage: node.find('mileage, odometer, miles').first().text(),
      color: node.find('color, exterior_color').first().text(),
      bodyStyle: node.find('body_style, bodytype, body').first().text(),
      ...
      photos: node.find('photo, image, picture').map((i, p) => $(p).text()).get(),
...
function parseHtmlJsonLd(html, feedUrl) {
...
function scrapeHtmlVehicles(html, feedUrl) {
  const $ = cheerio.load(html);
  const vehicles = [];

  $('.inventory-listing, .vehicle-card, [data-vin]').each((_, el) => {
    const node = $(el);
    const raw = {
      vin: node.attr('data-vin') || extractVin(node.text()),
      title:
        node.find('h1, h2, h3, h4, .title, .vehicle-title').first().text() || cleanText(node.text()),
...
module.exports = {
  async parse(feedUrl) {
    try {
      ...
      if (looksXml) {
        const xmlVehicles = parseXmlVehicles(text, feedUrl);
        console.log(`${LOG_PREFIX} extracted ${xmlVehicles.length} vehicles via XML`);
        return xmlVehicles;
      }

      const ldVehicles = parseHtmlJsonLd(text, feedUrl);
      if (ldVehicles.length) {
        console.log(`${LOG_PREFIX} extracted ${ldVehicles.length} vehicles via JSON-LD`);
        return ldVehicles;
      }

      const scrapedVehicles = scrapeHtmlVehicles(text, feedUrl);
